<?php
	...
	$id = $_REQUEST['id'];
	$currentNewsCount = $_REQUEST['currentNewsCount'];
	...
	if($currentNewsCount != count($fullDataNewsesUser)){
		$result = '';
		$idImage = 0;
		...
		foreach($fullDataNewsesUser as $news){
			...
			$result .= "</form>";
			$result .= "<div class=theDataSender><a href=?contactId=$news[idOfTheSender]><div class=windowAvatarSender><img data-needAvatarSize=".json_encode(processingPhoto($avatarPutch, 65))." class=avatarSender id=$idavatarSender src=$avatarPutch></div></a><a href=?contactId=$news[idOfTheSender]><p class=theFullNameSender>$dataOfTheSender[name] $dataOfTheSender[surname]</p></a></div>";
			if(!empty($news['text'])) $result .= "<p class=newsText>$news[text]</p>";
			if(!empty($news['attachedFile'])){ $result .= "<div class=newsWindowImage><img id=$idImageNews data-needNewsImageSize=".json_encode(processingPhoto("data/userImages/$id/userNewsImages/$news[attachedFile]", 550))." src=data/userImages/$news[userId]/userNewsImages/$news[attachedFile] class=newsImage></div>";
			}
			$result .= "</div>";
			echo $result;
